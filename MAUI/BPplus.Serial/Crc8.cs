namespace BPplus.Serial;

/// <summary>
/// CRC-8 implementation used by the BP+ device to validate XML and IDs_H payloads.
/// Algorithm by Mario Konrad (http://www.mario-konrad.ch/index.php?page=20005).
/// </summary>
public static class Crc8
{
    private const byte Init = 0xFF;

    private static readonly byte[] Lut =
    {
        ,
    };

    /// <summary>Computes the CRC-8 over <paramref name="data"/>.</summary>
    public static byte Compute(ReadOnlySpan<byte> data)
    {
        byte val = Init;
        foreach (var b in data)
            val = Lut[b ^ val];
        return val;
    }
}
